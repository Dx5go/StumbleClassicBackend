...
const BackendUtils = {
  ...
  createLoginHash: (deviceId, version, timestamp, stumbleId, steamTicket, scopelyId) => {
    const hashInput = `${deviceId}${version}${timestamp}${stumbleId || ''}${steamTicket || ''}${scopelyId || ''}${config.secrets.leagueSalt}`;
    return crypto.createHash('md5').update(hashInput).digest('hex');
  },
  createRegularHash: (deviceId, googleId, token, timestamp, stumbleId, url, body) => {
    const hashInput = `${deviceId}${googleId}${token}${timestamp}${stumbleId}${url}${body || ''}${config.secrets.leagueSalt}`;
    return crypto.createHash('md5').update(hashInput).digest('hex');
  },
  ...
  LeagueEncrypt: (text) => {
    const cipher = crypto.createCipheriv('aes-256-cbc', config.secrets.leagueSalt, crypto.randomBytes(16));
...
class Database {
  ...
  async connect() {
    ...
    if (this.mongoUri.includes("<username>")) {
...
async autoPopulateSharedData() {
    try {
        if (!config.flags.seedSharedDataOnStart) {
            return;
        }
...
class UserModel {
  ...
  static async create(deviceId, platformData = {}) {
    ...
    const username = 'BornPlayer-' + BackendUtils.GenCaracters(5).toUpperCase();
...
async function generatePhotonJwt(user) {
  ...
  const secret = config.secrets.jwtSecret || config.secrets.leagueSalt;
  const options = { expiresIn: '30d', issuer: config.appName };

  if (!secret) {
    throw new Error("JWT secret is missing. Configure JWT_SECRET or LEAGUE_SALT");
  }
...
static async updateUsername(req, res) {
  ...
  static async linkPlatform(req, res) {
    try {
      ...
      const platformIdMD5 = BackendUtils.Hash('md5', platformId || `${platform}-${user.username}-${config.secrets.leagueSalt}`);
...
class MatchmakingController {
  static async getMatchmakingFilter(req, res) {
    try {
      ...
      const sharedType = config.shared.type || 'NULL';
